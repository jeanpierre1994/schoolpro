    ...
    <div class="pagetitle">
        ...
        <nav>
            <ol class="breadcrumb">
                ...
                <li class="breadcrumb-item"><a href="{{ url()->previous() }}" style="text-decoration: none;">Liste
                        Etudiants</a></li>
                ...
                <div class="card">
                    <div class="card-body">
                        ...
                        <h5 class="card-title">
                            ...
                            Groupe pédagogique : <b style="color: black;">{{ $gp->getPole->libelle }}
                                {{ $gp->getFiliere->libelle }} {{ $gp->libelle_classe }} {{ $gp->libelle_secondaire }}</b>
                            <br><br>
                            Etudiant : <b style="color: black;">{{ $etudiant->matricule }}
                                {{ $etudiant->getDossier->getPersonne->nom }}
                                {{ $etudiant->getDossier->getPersonne->prenoms }}</b>
                        </h5>
                        <!-- Bordered Table -->
                                    ...
                                    @foreach ($examens as $item)
                                        <tr>
                                            ...
                                            <td class="text-center">
                                                <a target="_blank"
                                                    href="{{ route('pdf', ['id' => \Crypt::encrypt($item->id), 'gp_id' => \Crypt::encrypt($gp->id), 'etudiant_id' => \Crypt::encrypt($etudiant->id)]) }}"
                                                    title="Relevé de note"><button type="button"
                                                        class="btn btn-sm btn-danger"><i
                                                            class="bi bi-file-earmark-pdf text-white" style="color: white"
                                                ...
                                                <form
                                                    ...
                                                    @csrf
                                                    @method('PATCH')
                                                    <input type="hidden" name="etudiant_id" value="{{ \Crypt::encrypt($etudiant->id) }}">
